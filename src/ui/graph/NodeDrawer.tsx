  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3 border-b hairline p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-[17px] text-parchment-50">{node.label}</h3>
            {node.userAdded && (
          ...
          <p className="text-[12.5px] text-parchment-300/65">{node.subtitle}</p>
          <div className="mt-1 flex items-center gap-2 text-[10.5px] uppercase tracking-wide text-parchment-300/45">
            ...
            {excluded && <span className="text-parchment-300/50">· set aside</span>}
          </div>
        </div>
        <button
          onClick={() => select(undefined)}
          className="rounded-lg p-1.5 text-parchment-300/60 transition hover:bg-white/8 hover:text-parchment-50"
        >
          <X size={17} />
        </button>
      </div>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {/* insight */}
        <p className="text-balance text-[13.5px] leading-relaxed text-parchment-100">{node.insight}</p>
        ...
        {node.payDescription && (
          <div className="rounded-lg bg-ink-950/40 p-2.5">
            <div className="text-[10.5px] uppercase tracking-wide text-parchment-300/50">Starting pay</div>
            <div className="mt-0.5 text-[12.5px] leading-relaxed text-parchment-100">{node.payDescription}</div>
          </div>
        )}
        {node.entrancePrepCost && (
          <div className="rounded-lg bg-ink-950/40 p-2.5">
            <div className="text-[10.5px] uppercase tracking-wide text-parchment-300/50">Entrance prep</div>
            <div className="mt-0.5 text-[12.5px] leading-relaxed text-parchment-100">{node.entrancePrepCost}</div>
          </div>
        )}
        ...
        {node.eligibilityThreshold && (
          <div className="rounded-lg border border-white/8 p-2.5">
            ...
            <div className="text-[12.5px] text-parchment-100">{node.eligibilityThreshold.typicalValue}</div>
            <div className="mt-1 text-[11.5px] leading-relaxed text-parchment-300/60">
        ...
        {node.checklist.length > 0 && (
          <div>
            <div className="mb-1.5 flex items-center gap-1.5 text-[11px] uppercase tracking-wide text-parchment-300/60">
              ...
              {node.checklist.map((c, i) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-start gap-2 rounded-lg p-1.5 text-left transition hover:bg-white/5"
                  >
                    <span
                      className={cn(
                        'mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border',
                        done ? 'border-signal-confirm bg-signal-confirm text-ink-950' : 'border-white/20',
                      )}
                    >
                    ...
                    <span
                      className={cn(
                        ...
                        done ? 'text-parchment-300/50 line-through' : 'text-parchment-100',
                      )}
                    >
                      {c}
                    </span>
          ...
          <div>
            ...
            <ul className="space-y-1">
              {node.opportunities.map((o, i) => (
                <li key={i} className="flex gap-2 text-[12.5px] leading-snug text-parchment-100">
      ...
      <div className="space-y-2 border-t hairline p-4">
        {isChallengeable && (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-amber/90 px-4 py-2.5 text-[13.5px] font-semibold text-ink-950 transition hover:bg-amber disabled:opacity-40"
          >
            <Swords size={15} />
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-2 text-[12.5px] text-parchment-300/75 transition hover:bg-white/5"
        >
...
function Stat({ label, value, small }: { label: string; value: string; small?: boolean }) {
  return (
    <div className="rounded-lg bg-ink-950/40 p-2 text-center">
      <div className="text-[9.5px] uppercase tracking-wide text-parchment-300/50">{label}</div>
      <div className={cn('mt-0.5 font-semibold text-parchment-50', small ? 'text-[11px]' : 'text-[13px]')}>
        {value}
      </div>
    </div>
  )
...
function Tool({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 rounded-lg border border-white/8 bg-white/[0.03] px-2.5 py-2 text-[12px] text-parchment-100 transition hover:border-amber/30 hover:bg-amber/5"
    >
